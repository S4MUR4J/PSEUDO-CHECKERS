...
class Game:
    ...
    def __fill_board(self) -> None:
        ...
        # Empty center of the board depends on board size
    ...
    # If a capture is possible, one of the pieces must make it
    ...
    def all_possible_moves(self) -> list[tuple[Vector2, Vector2]]:
        ...
        # During multi-capture, only the piece that started the chain may move
        ...
        # Checked only on the player's first move in a turn, not during multi-capture
    ...
    def play_turn(
        ...
        self.__move_checker(old_pos, new_pos, real_move)
        self.__are_checkers_on_board()

        if self.is_end_game:
            return

        # After a capture, if another is possible — force the next move with the same piece
